...
pub fn module_instanciate<'a >(store: &'a mut Store, module: Module, externvals: Vec<ExternVal>) -> (&'a mut Store, Result<ModuleInst, Error>) {
    let (store, frame, v) = module.instanciate(store, externvals);
    if v.is_empty() {
        (store, Ok(frame.module))
    } else {
...
impl Module {
    fn instanciate<'a>(&self, store: &'a mut Store, externvals: Vec<ExternVal>) -> (&'a mut Store, Frame, Vec<Instr>) {
        let frame = Frame::default();
        let externtypes = match self.validate() {
            Err(error) => {
                return (store, frame, vec![Instr::Trap]);
            },
            ...
            match ext_val {
                ExternVal::Func(a) => {
                    let functype = match store.funcs.get(a.clone()) {
                        None => return (store, frame, vec![Instr::Trap]),
